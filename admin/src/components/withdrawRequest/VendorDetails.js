...
const VendorDetails = () => {
    ...
    const { data, isLoading, refetch } = useCustomQuery([VendorTransactions, role], () => request({ url: VendorTransactions },router), { enabled: false, refetchOnWindowFocus: false, select: (data) => data.data });
    useEffect(() => {
        const storedRole = localStorage.getItem("role");
        let parsedRoleName = null;
        if (storedRole && storedRole !== "undefined" && storedRole !== "null") {
            try {
                parsedRoleName = JSON.parse(storedRole)?.name;
            } catch (e) {
                localStorage.removeItem("role");
            }
        }
        if (parsedRoleName === 'vendor') {
            refetch()
        }
        setRole(parsedRoleName)
    }, [])
    return (
        <>
