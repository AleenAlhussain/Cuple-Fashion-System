...
const VendorProfile = () => {
  ...
  const { mutate, isLoading } = useCreate(VendorSettingAPI, false, "/account", false);
  return (
    <Formik
      enableReinitialize
      initialValues={{
      ...
      {({ setFieldValue, values, errors, touched }) => (
        <Form className="theme-form theme-form-2 mega-form">
          <Row>
            ...
            <SimpleInputField
              nameList={[
                { name: "store_name", placeholder: t("EnterStoreName") },
                { name: "description", title: "StoreDescription", type: "textarea", placeholder: t("EnterDescription") },
              ]}
            />
            <AddressComponent values={values} isRequired={false} />
            <SimpleInputField
              nameList={[
