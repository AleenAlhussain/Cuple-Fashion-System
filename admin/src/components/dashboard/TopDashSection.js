...
const TopDashSection = ({ role }) => {
  ...
  const { data, refetch } = useCustomQuery([StatisticsCountAPI], () => request({ url: StatisticsCountAPI, params: { filter_by: filterValue } }, router), { refetchOnWindowFocus: false, select: (res) => res?.data?.data });
  useEffect(() => {
    if (filterValue != null) {
      refetch();
    }
      ...
      <Container fluid={true} className="p-0">
        <Row className="g-sm-4 g-3">
          <Col md="4" className="welcome-tiles">
            <Card className="m-0">
              <Image height={162} width={497} src={"/assets/images/settings/footer-logo.png"} className="img-fluid" alt="box-time" />
              <CardBody>
