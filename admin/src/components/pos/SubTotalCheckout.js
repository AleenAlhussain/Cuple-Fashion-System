                ...
                {!values['products']?.length ? (
                    ...
                    <>
                        ...
                        <ul className={`summary-total`}>
                            {(() => {
                                const computedSubtotal = (values?.products || []).reduce((sum, item) => {
                                    const qty = Number(item?.quantity ?? 1);
                                    const rawFinal = item?.final_price ?? item?.price ?? item?.product?.final_price ?? item?.product?.price ?? 0;
                                    const finalNum = Number(rawFinal);
                                    const unit = finalNum > 0 ? finalNum : Number(item?.price ?? item?.product?.price ?? 0);
                                    return sum + (isNaN(unit) ? 0 : unit) * (isNaN(qty) ? 1 : qty);
                                }, 0);
                                const displayTotal = values?.total && Number(values.total) > 0 ? Number(values.total) : computedSubtotal;
                                return (
                                    <>
                                        <li>
                                            <h4>{t("Subtotal")}</h4>
                                            <h4 className="price">{convertCurrency(displayTotal)}</h4>
                                        </li>
                                        <li>
                                            <h4>{t("Shipping")}</h4>
