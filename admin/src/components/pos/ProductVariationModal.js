...
const ProductVariationModal = ({ productData, isLoading, setFieldValue, setModal, mutate }) => {
  ...
  const { data: productDetails } = useCustomQuery(
    [product, productData?.id],
    () => request({ url: `${product}/${productData?.id}` }, router),
    {
  ...
  const wholesalePriceCal = () => {
    const wholesales = cloneVariation?.product?.wholesales || [];
    let wholesale = wholesales.find((value) => value.min_qty <= cloneVariation.productQty && value.max_qty >= cloneVariation.productQty) || null;
  ...
  useEffect(() => {
    if (productDetails) {
      const normalizedProduct = {
        ...productDetails,
        variations: productDetails?.variations ?? productDetails?.variants ?? [],
      };
      setCloneVariation((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    if (cloneVariation?.product) {
      wholesalePriceCal();
      checkVariantAvailability(cloneVariation.product);
    }
