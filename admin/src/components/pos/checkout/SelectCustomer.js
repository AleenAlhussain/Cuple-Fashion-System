...
const SelectCustomer = ({ values, setFieldValue, userData, userRefetch, setSearch, userLoader }) => {
  ...
  const { mutate: userCreate, isLoading } = useCreate(user, false, false, "Customer created Successfully", () => {
    userRefetch();
  ...
  return (
    <CheckoutCard icon={<RiUserLine />}>
      ...
      <ShowModal modalAttr={{ className: "modal-lg" }} title={"Addcustomer"} open={modal} setModal={setModal}>
        <UserForm mutate={userCreate} loading={isLoading} noRoleField fixedRole posMinimal />
      </ShowModal>
    </CheckoutCard>
  );
};
