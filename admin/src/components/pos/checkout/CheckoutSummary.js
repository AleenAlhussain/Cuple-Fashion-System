...
const CheckoutSidebar = ({ addToCartData, values, setFieldValue, data, loading, mutate, userData, errorCoupon, appliedCoupon, setAppliedCoupon, storeCoupon, setStoreCoupon }) => {
  ...
  const fallbackSubtotal =
    addToCartData?.subtotal ??
    addToCartData?.items?.reduce((sum, item) => sum + (item?.final_price ?? item?.price ?? 0) * (item?.quantity ?? 1), 0) ??
    0;
  const hasShippingAddress = Boolean(values?.shipping_address_id);
  const fallbackShipping = hasShippingAddress ? 0 : null;
  const fallbackTax = hasShippingAddress ? 0 : null;
  const fallbackTotal = hasShippingAddress ? (fallbackSubtotal + (fallbackShipping || 0) + (fallbackTax || 0)) : null;
    ...
    <Card className="pos-detail-card">
      <div className="pos-loader">
        ...
        <ul className={`summary-total`}>
          <li>
            <h4>{t("Subtotal")}</h4>
            <h4 className="price">{data?.data?.total?.sub_total ? convertCurrency(data?.data?.total?.sub_total) : convertCurrency(fallbackSubtotal)}</h4>
          </li>
          <li>
            <h4>{t("Shipping")}</h4>
            <h4 className="price">{data?.data?.total?.shipping_total >= 0 ? convertCurrency(data?.data?.total?.shipping_total) : (fallbackShipping === null ? "—" : convertCurrency(fallbackShipping))}</h4>
          </li>
          <li>
            <h4>{t("Tax")}</h4>
            <h4 className="price">{data?.data?.total?.tax_total ? convertCurrency(data?.data?.total?.tax_total) : (fallbackTax === null ? "—" : convertCurrency(fallbackTax))}</h4>
          </li>
          ...
          <li className="list-total">
            <h4>{t("Total")}</h4>
            <h4 className="price">{data?.data?.total?.total ? convertCurrency(data?.data?.total?.total) : (fallbackTotal === null ? "—" : convertCurrency(fallbackTotal))}</h4>
          </li>
        </ul>
      </div>
