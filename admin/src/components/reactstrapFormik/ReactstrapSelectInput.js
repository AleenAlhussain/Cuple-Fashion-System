...
const ReactstrapSelectInput = ({ field, form: { touched, errors, setFieldValue }, getValuesKey = "id", setvalue,noSearchBar, ...props }) => {
  ...
  useEffect(() => {
    ...
    if (searchInput) {
      if (props.inputprops?.setsearch) {
        ...
        setList(
          options.filter((item) => item?.name?.toLowerCase().includes(searchInput?.toLowerCase() || "")),
        );
      }
    } else {
  ...
  useEffect(() => {
    if (props?.inputprops?.value !== undefined) {
      setSelectedItems(props.inputprops.value);
    }
  ...
  }, [listOpt, field?.value]) // Re-run when options are loaded or field value changes
            ...
            {(props.inputprops?.setsearch ? listOpt : list)?.map((option, index) => (
              <Fragment key={index}>
                {option?.data ?
                  ...
                  : <li
                    className={field?.name === "role_id" && option?.id === "shop_manager" ? "role-option role-option-shop-manager" : ""}
                    onClick={() => onSelectValue(option)}
                  >
                    {option?.image && (
                      <Image
                        src={resolveMediaUrl(option?.image) || "/assets/images/placeholder/collection_category.png"}
