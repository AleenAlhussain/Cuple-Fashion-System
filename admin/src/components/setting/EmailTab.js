...
const EmailTab = ({ values, setFieldValue, errors }) => {
  const router = useRouter();
  const [sendingTestEmail, setSendingTestEmail] = useState(false);

  const handleNonSubmitButton = async () => {
    setFieldValue("submitButtonClicked", true);

    const toMail = values?.email?.trim();
    if (!toMail) {
      ToastNotification("error", "Please enter a valid recipient email.");
      return;
    }

    const payload = {
      to_mail: toMail,
      mail_encryption: values?.mail_encryption || values?.values?.email?.mail_encryption,
      mail_mailer: values?.mail_mailer || values?.values?.email?.mail_mailer,
      mail_from_address: values?.values?.email?.mail_from_address,
      mail_from_name: values?.values?.email?.mail_from_name,
    ...
    try {
      const response = await request(
        { url: "/settings/test-email", method: "post", data: payload },
        router
      );
      ToastNotification(
        "success",
  ...
  return (
    <>
      ...
      <button
        type="button"
        name="email"
        disabled={sendingTestEmail || !!errors?.email || !values?.email?.trim()}
        title="SendEmail"
        className="btn btn-animation  ms-auto"
        onClick={handleNonSubmitButton}
      >
        {sendingTestEmail ? "Sending..." : t("SendEmail")}
      </button>
