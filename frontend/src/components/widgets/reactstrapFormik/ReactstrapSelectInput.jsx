...
const ReactstrapSelectInput = ({ field, form: { touched, errors, setFieldValue }, getValuesKey = "id", multiple = false, setvalue, ...props }) => {
  ...
  useEffect(() => {
    const source = props.inputprops?.setsearch ? listOpt : list;
    if (!Array.isArray(source)) return;

    if (Array.isArray(field?.value)) {
      const selected = source.filter((elem) =>
        field?.value?.some((value) => String(value) === String(elem[getValuesKey]))
      );
    ...
    const selected = source.find((elem) => String(field?.value) === String(elem[getValuesKey]));
    setSelectedItems(selected);
  }, [field?.value, getValuesKey, list, listOpt, props.inputprops?.setsearch]);
  ...
  return (
    <>
      ...
      <div className="custom-select-box cursor-pointer" ref={ref}>
        ...
        {!Array.isArray(selectedItems) && <Input id={props.inputprops.id} {...field} {...props} value={field.value ?? ""} placeholder={t("Search")} className="form-control form-select" type="text" invalid={Boolean(touched[field.name] && errors[field.name])} disabled />}
                      ...
                      >
                        <div className="country">
                          <div className="flag-box">
                            <div className={`${option?.data?.class} iti-flag`}></div>
                          </div>
