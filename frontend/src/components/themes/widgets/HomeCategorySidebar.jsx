...
import { ImagePath, localizedValue } from "@/utils/constants";
...
const HomeCategorySidebar = ({ categoryIds, height, width, style, slider, sliderOptions }) => {
  ...
  const cn = (category) => localizedValue(category, 'name', lang);
          ...
          {style == "vertical" && (
            <ul className="sm pixelstrap sm-vertical" id="sub-menu">
              {mainCategories?.map((category, index) => (
                <li key={index}>
                  <Link href={`/category/${category?.slug}`}>{cn(category)}</Link>{" "}
                </li>
              ))}
            </ul>
          )}
                  ...
                  <Link href={`/category/${category?.slug}`}>
                    <div className="category-boxes">
                      <div className="img-sec">
                        <img height={height ? height : 58} width={width ? width : 58} src={category.category_icon ? category.category_icon.original_url : `${ImagePath}/placeholder/category.png`} className="img-fluid" alt={cn(category)} />
                  ...
                  <Link href={`/category/${category?.slug}`}>
                    <div className={`gradient-category ${i == 1 ? "hover-effect" : ""}`}>
                      <div className="gradient-border">
                        <div className="img-sec">
                          <img height={height ? height : ""} width={width ? width : ""} src={category.category_image ? category?.category_image?.original_url : `${ImagePath}/placeholder/category.png`} className="img-fluid" alt={cn(category)} />
                        </div>
                      </div>
                      <h4>{cn(category)}</h4>
                    </div>
                  ...
                  <div className="category-wrapper">
                    <div>
                      {category?.category_image ? (
                        <div>
                          <img src={category?.category_image?.original_url} className="img-fluid" alt={cn(category)} />
                        </div>
                      ) : (
                        <div>
                          <img src={`${ImagePath}/tools/category/1.jpg`} className="img-fluid" alt={cn(category)} />
                        </div>
                      )}
                      <h4>
                        <Link href={`/category/${category?.slug}`}>{cn(category)}</Link>
                      ...
                      <ul className="category-link">
                        {category?.subcategories
                          ? category?.subcategories?.slice(0, 5)?.map((sub) => (
                              <li>
                                <Link href={`/category/${sub?.slug}`}>{localizedValue(sub, 'name', lang)}</Link>
                              </li>
                            ))
                          : ""}
                      </ul>
                      ...
                      <div key={i} className="category-block">
                        <Link href={`/category/${category?.slug}`}>
                          <div className="category-image svg-image">
                            <div className="img-sec">
                              <img src={category.category_icon ? category.category_icon.original_url : `${ImagePath}/placeholder/category.png`} className="img-fluid" alt={cn(category)} />
                            </div>
                          </div>
                        </Link>
                ...
                <Col sm="4" key={i} className=" border-padding">
                  <div className="category-banner">
                    <div className="img-fluid lazyload bg-img" style={{ backgroundImage: `url(${category.category_image ? category?.category_image?.original_url : `${ImagePath}/placeholder/category.png`}` }}>
                      <img src={category.category_image ? category?.category_image?.original_url : `${ImagePath}/placeholder/category.png`} className="img-fluid lazyload bg-img" alt={cn(category)} />
                    </div>
                    ...
                    <Slider {...categorySliderSettingMain}>
                      {mainCategories.map((category, index) => (
                        <div className="category-block" key={index}>
                          <Link href={`/category/${category?.slug}`}>
                            <div className="category-image">
                              <img src={category.category_image ? category.category_image.original_url : `${ImagePath}/placeholder/category.png`} className="img-fluid" alt={cn(category)} />
                            </div>
                          </Link>
                          <div className="category-details">
                            <Link href={`/category/${category?.slug}`}>
                    ...
                    <Link href={`/category/${category?.slug}`}>
                      <div className="img-category">
                        <div className="img-sec bg-size" style={{ backgroundImage: `url(${category.category_image ? category.category_image.original_url : `${ImagePath}/placeholder/category.png`})` }}>
                          <img src={category.category_image ? category.category_image.original_url : `${ImagePath}/placeholder/category.png`} className="img-fluid bg-img" alt={cn(category)} />
                        </div>
                        <h4>{cn(category)}</h4>
                      </div>
                    </Link>
