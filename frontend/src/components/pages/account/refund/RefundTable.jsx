...
const RefundTable = () => {
  ...
  const { data, isLoading, refetch } = useFetchQuery(
    [RefundAPI],
    () => axios({ url: RefundAPI, params: { page, paginate: 10 } }),
    {
      enabled: false,
  ...
  const refunds = useMemo(() => data?.data ?? [], [data]);
  const summaryItems = useMemo(() => getHistorySummaryItems(refunds, "refund"), [refunds]);
  ...
  const hasRecords = Boolean(refunds.length);

  return (
    <Card className="dashboard-table mt-0 history-card">
      <CardBody className="p-0">
        <AccountHeading title="RefundHistory" classes={"top-sec"} />
        ...
        {hasRecords ? (
                    ...
                    return (
                      <tr key={refund?.id ?? `${index}-${orderNumber}`}>
                        ...
                        <td className="history-table__date">
                          {showMonthWiseDate(refund?.created_at)}
                        </td>
                      </tr>
                    );
                  })}
          ...
          <div className="history-empty-state">
            <NoDataFound
              ...
              description="NoRefundsYetDescription"
              height="300"
              width="300"
            />
